...
!!! note "Not every provider class supports this"
    `BedrockConverseModel` and `GoogleAIGenerateModel` are **not** part of this mechanism — they authenticate via AWS SigV4 / Google's own SDK conventions, which have no simple bearer-token `base_url` override.
