# Compatible Endpoints — AWS Bedrock Mantle Example

Demonstrates `OpenAIChatModel`, `OpenAIResponsesModel`, and `AnthropicMessagesModel` all pointed at AWS Bedrock's [`bedrock-mantle`](https://docs.aws.amazon.com/bedrock/latest/userguide/bedrock-mantle.html) endpoint via `api_key`/`base_url`/`default_headers` — no llmfy code changes required. See [Compatible Endpoints](../documentation/features/compatible-endpoints.md) for the full explanation of how this works and its caveats.
...
BEDROCK_MANTLE_WORKSPACE_ID=...         # optional — scopes Messages API calls to a Bedrock Workspace
BEDROCK_MANTLE_CHAT_MODEL=...           # optional — overrides the Chat Completions example's model
BEDROCK_MANTLE_RESPONSES_MODEL=...      # optional — overrides the Responses API example's model
BEDROCK_MANTLE_MESSAGES_MODEL=...       # optional — overrides the Messages API example's model
BEDROCK_MANTLE_RESPONSES_BASE_URL=...   # optional — overrides the Responses example's base_url (see warning below)
```

!!! warning "Model/API availability can't be predicted from docs alone — verify per account"
    Model availability on `bedrock-mantle` is both per-account (accounts provisioned through an AWS Solution Provider / Distributor — "channel program" accounts — may not have every model enabled) **and** per-API-surface (AWS's own [API compatibility by models](https://docs.aws.amazon.com/bedrock/latest/userguide/models-api-compatibility.html) table can say a model supports an API, yet your specific account/model combination still gets rejected with `"...does not support the '/v1/...' API"`). There is no single call that confirms "my account can reach model X on surface Y" ahead of time.

    Run `bedrock_mantle_list_available_models_example()` first to see which model IDs your account can reach at all, then set the `BEDROCK_MANTLE_*_MODEL` env vars above to whichever ones actually work for the surface you need — don't rely on the hardcoded defaults in this script.

!!! warning "Not every model shares the same base_url path on bedrock-mantle"
    `gpt-oss-120b`'s [model card](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-openai-gpt-oss-120b.html) lists its `bedrock-mantle` URL as `.../v1`. GPT-5.6 Luna's [model card](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-openai-gpt-56-luna.html) explicitly says it's served on `.../openai/v1` instead — *"This is different from the v1/responses path used by other models on the responses endpoint."* There's no way to know a model's required prefix except by checking its own model card's "Programmatic Access" table. This is why the Responses example below uses a separate `RESPONSES_BASE_URL` (defaulting to the `/openai/v1` path Luna needs) rather than reusing the Chat Completions example's `OPENAI_COMPAT_BASE_URL` — if you swap in a different Responses model via `BEDROCK_MANTLE_RESPONSES_MODEL`, check its model card and override `BEDROCK_MANTLE_RESPONSES_BASE_URL` if it uses the plain path instead.

!!! warning "Some models reject `temperature`/`top_p` outright"
    GPT-5.6 Luna returns a 400 `unsupported_parameter` for `temperature` (and would for `top_p` too) rather than silently accepting a default — the same category of restriction OpenAI's own reasoning models (o-series, GPT-5.x) apply. As of this example, `OpenAIChatConfig`/`OpenAIResponsesConfig` accept `temperature: float | None` / `top_p: float | None` — passing `None` **omits the field from the request entirely** instead of sending a default or an explicit `null`. The Responses example below uses `OpenAIResponsesConfig(temperature=None, top_p=None)` for exactly this reason.
