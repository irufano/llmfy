"""Unit tests for llmfy/llmfy_core/messages/message_temp.py."""
...
@pytest.fixture
def temp() -> MessageTemp:
    return MessageTemp()


class TestAddSystemMessage:
    def test_inserted_at_front(self, temp: MessageTemp):
...
class TestAddUserMessage:
    def test_appended_with_given_id(self, temp: MessageTemp):
...
class TestAddAssistantMessage:
    def test_appended_message(self, temp: MessageTemp):
    ...
    def test_mutates_tool_call_request_call_id_in_place(self, temp: MessageTemp):
        tool_call = ToolCall(
    ...
    def test_no_tool_calls_does_not_raise(self, temp: MessageTemp):
        temp.add_assistant_message("a1")
        assert temp.messages[-1].tool_calls is None


class TestAddToolMessage:
    def test_delegates_to_real_formatter_and_appends(self, temp: MessageTemp):
    ...
    def test_unsupported_backend_raises_llmfy_exception(
        self, temp: MessageTemp, monkeypatch
    ):
        monkeypatch.setattr(
            MessageTemp, "_get_formatter", classmethod(lambda cls, backend: None)
        )
...
class TestGetMessages:
    def test_unsupported_backend_raises(self, temp: MessageTemp, monkeypatch):
        monkeypatch.setattr(
            MessageTemp, "_get_formatter", classmethod(lambda cls, backend: None)
        )
        with pytest.raises(LLMfyException, match="Unsupported model backend"):
    ...
    def test_formats_every_message_in_order(self, temp: MessageTemp):
        temp.add_user_message("u1", "hi")
    ...
    def test_format_message_is_cached_per_id(self, temp: MessageTemp, monkeypatch):
        formatter = MessageTemp._get_formatter(ModelBackend.OPENAI_CHAT)
        call_count = {"n": 0}
    ...
    def test_cache_evicted_after_clear(self, temp: MessageTemp):
...
class TestGetInstanceMessages:
    def test_returns_live_reference_not_a_copy(self, temp: MessageTemp):
        temp.add_user_message("u1", "hi")
...
class TestClear:
    def test_no_system_message_leaves_history_empty(self, temp: MessageTemp):
        temp.add_user_message("u1", "hi")
        temp.clear()
        assert temp.messages == []

    def test_keeps_the_frontmost_system_message(self, temp: MessageTemp):
        temp.add_system_message("first")
        temp.add_user_message("u1", "hi")
